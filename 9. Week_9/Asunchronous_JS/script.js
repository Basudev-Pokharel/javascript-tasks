...
function processOrderPromise() {
  delay("Order received", 2000)
    .then((message) => {
      console.log(message);
      return delay("Preparing order", 3000);
    })
    .then((message) => {
      console.log(message);
      return delay("Order ready for pickup", 1000);
    })
    .then(console.log)
    .catch(console.error);
}
...
async function checkoutCart() {
  console.log(await delay("Checking stock...", 1000));
  console.log(await delay("Payment processed", 2000));
  console.log(await delay("Order completed!", 1000));
}
function delay(message, time) {
  return new Promise((resolve) => setTimeout(() => resolve(message), time));
}
